...
AddEventHandler('esx_SellGold:process', function()
	if not playersProcessing[source] then
		...
		playersProcessing[_source] = ESX.SetTimeout(10000, function()
			...
			local xPlayer = ESX.GetPlayerFromId(_source)
			local xGoldo, xGoldt = xPlayer.getInventoryItem('gold_o'), xPlayer.getInventoryItem('gold_t')
			
			if xCrack.xGoldo > 10 then
				if xPlayer.canSwapItem('gold_o', 10, 'gold_t', 1) then
					xPlayer.removeInventoryItem('crack', 10)
